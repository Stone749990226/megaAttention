...
FlashAttention 的自然 tile 是：

```text
FA task = (row_key, head)
        = (batch_idx, m_block, head)
```

一个 task 产出：

```text
O_scratch[row_key, m, head, d]
```

这里：

```text
row_key   : 一个 varlen sequence 内的一组 Q rows，逻辑上等于 (batch_idx, m_block)
batch_idx : 当前 serving batch 中的序列编号
m_block   : 当前 sequence 内的 Q tile 编号，不跨 sequence 递增
head      : 当前 TP rank 上的 local attention head
m         : row tile 内的位置
d         : head_dim 维度
```

物理存储可以继续使用压平后的 row tile id：

```text
row_tile_id = cu_m_blocks[batch_idx] + m_block
cu_m_blocks[b] = sum_{i < b} ceil(seqlen_q[i] / M_TILE)

O_scratch physical layout = [row_tile_id, m, head, d]
```

但所有依赖关系都按逻辑 `row_key = (batch_idx, m_block)` 理解。这样 causal mask、最后一个 partial Q tile、以及不同 sequence 的边界都不会被全局 row tile 混在一起。

O_proj 的输入不是单个 head，而是同一个 row tile 的所有 local heads 拼起来：

```text
O_row_tile_local = view O_scratch[row_tile_id, :, :, :]
                 as [M_TILE, H_local * D]
```

因此第一版固定 `O_scratch` 物理 layout 为 `[row_tile_id, m, head, d]`。这样 O_proj 的 A 矩阵可以把 `(head, d)` 视为连续 K 维；FA 每个 head 写出时按 `m` 行写入对应 head slice。
...
O_row_tile_local : [64, 4 * 128] = [64, 512]
W_o_local        : [512, 4096]
Y_partial        : [64, 4096]
```
...
```text
O_proj task = (row_key, out_n_tile)
            = (batch_idx, m_block, out_n_tile)

[M_TILE, H_local * D] @ [H_local * D, N_TILE]
    -> [M_TILE, N_TILE]
```
...
O_proj CTA task     = (row_key, n_super_group)
```
...
所以 FA 的计算粒度保持 `(row_key, head)`，但 O_proj 的启动条件是：

```text
同一个 row_key 的所有 local heads 都已经写入 O_scratch
```
...
FA task 队列在 kernel 启动时天然存在：

```text
num_row_tiles = cu_m_blocks[num_batch]
total_fa_tasks = num_row_tiles * H_local
```

一维 task id 先拆成压平 row tile 和 head：

```text
fa_task_id = row_tile * H_local + head

row_tile = fa_task_id / H_local
head     = fa_task_id % H_local
```

然后通过预生成的 row descriptor 映射到 varlen 坐标：

```text
row_key = row_desc[row_tile]
        = (batch_idx, m_block)
```

`row_desc[row_tile]` 是第一版设计里的固定元数据表，在 host 侧生成。它只保存压平 row tile 到 varlen 逻辑坐标的最小映射：
...
主 persistent kernel 不在热路径里按 `cu_seqlens_q` 反复做 prefix-sum 查找。CTA 领取到 `fa_task_id` 后，只需要一次读取 `row_desc[row_tile]`，就能得到该 tile 属于哪个 sequence、以及是 sequence 内第几个 Q tile。其它派生信息仍从 `cu_seqlens_q/cu_seqlens_k` 计算：
...
q_tile_start = q_start + m_block * M_TILE
valid_m      = min(M_TILE, q_len - m_block * M_TILE)
```

这样 `row_desc` 只负责调度 id 到 varlen 坐标的还原，不缓存 offset、length、`valid_m` 等可由 `cu_seqlens` 直接推出的信息。
...
O_proj task 不是一开始全部生成。某个 `row_key` 的所有 heads 完成后，最后完成的 FA CTA 将该 row key 的 O_proj tasks 写入 ready queue：

```text
(row_key, n_super_group = 0)
(row_key, n_super_group = 1)
...
(row_key, n_super_group = num_super_groups - 1)
```

这样 O_proj CTA 不需要自旋等待 `row_ready`。只要某个 task 的 queue entry 已经通过 `oproj_publish_tail` release 发布，就说明对应 row key 的 `O_scratch` 已经可以读取。

### O_proj 规模估算

一个 `row_tile` 最多包含 `M_TILE` 个 token。第一版按：
...
```text
B=1,  L=32k  -> total tokens=32k  -> row_tiles=512
B=4,  L=16k  -> total tokens=64k  -> row_tiles=1024
B=8,  L=8k   -> total tokens=64k  -> row_tiles=1024
B=16, L=4k   -> total tokens=64k  -> row_tiles=1024
```
...
```text
super_group_n_tiles=4 -> 每 row 8 个 O_proj task / AR tile
super_group_n_tiles=8 -> 每 row 4 个 O_proj task / AR tile
```
...
batch_idx       = row_desc[row_tile_id].batch_idx
...
ready_count:
    仍然按 ar_slot_id = (row_tile_id, n_super_group) 每 rank 加一次
```
...
N_TILE = 128
total_tokens = 64k
num_row_tiles = total_tokens / M_TILE = 1024
...
bitset 方案的 producer 很便宜。某个 row 的所有 heads 完成后，最后完成的 FA CTA 通常只需要一次 `atomicOr_release` 就能发布该 row 的所有 O_proj tasks：

```text
base_slot = row_tile_id * num_super_groups
mask      = 该 row 在 64-bit word 内对应的 n_super_group bits
...
某个 `row_key` 的所有 heads 完成后，最后完成的 FA CTA 作为 O_proj task producer。它一次性发布该 row 的所有 `num_super_groups` 个 O_proj tasks：

```text
publish_oproj_tasks(row_tile_id):
    n = num_super_groups
    start = atomicAdd(oproj_reserve_tail, n)
    end = start + n

    for i in 0 .. n - 1:
        slot_id = row_tile_id * num_super_groups + i
...
O_proj task:
    ...
    任务: slot_id -> (row_tile, n_super_group)

AR owner task:
    来源: ar_owner_probe_bits
    任务: ar_slot_id -> (row_tile, n_super_group)
```
...
```text
ar_slot_id = row_tile_id * num_super_groups + n_super_group
...
WG1/WG2:
    ...
    online softmax
    PV WGMMA
    维护 row_max、row_sum、acc_O
```

FA 主流程：

```text
取 FA task: (row_key, head)
...
WG1/WG2:
    ...
    rescale acc_O
    将 acc_O 写入 O_scratch[row_tile_id, :, head, :]

一个 elected lane:
    等待 O_scratch store 真正完成
    old = atomicAdd_acq_rel(head_ready_count[row_key], 1)
    如果 old + 1 == H_local:
        acquire 已完成所有 head 的 O_scratch 发布
        reserve/write/publish 该 row_key 的 O_proj queue entries
```
...
`head_ready_count[row_key]` 不只是普通计数器，也是 FA producer 和 O_proj task producer 之间的同步点。它必须使用 device-scope release/acquire 语义的 atomic；不能把普通 `atomicAdd` 默认当作数据发布协议。

每个 FA CTA 对一个 `(row_key, head)` 完成后：

```text
1. 等待所有 WGMMA 完成。
2. 完成 O_scratch[row_tile_id, :, head, :] 写出。
3. 如果使用 TMA S2G store，必须 wait 到 TMA store completion。
   注意：TMA 发起不等于 TMA 写完。
4. 执行 atomicAdd_acq_rel(head_ready_count[row_key], 1)。
```

这里的 release 语义保证：

```text
O_scratch[row_tile_id, :, head, :] 的写入
    happens-before
head_ready_count[row_key] 的递增被后续 acquire 观察到
```
...
因此它可以安全地发布该 `row_key` 的 O_proj queue entries。O_proj consumer 通过 acquire 读取 `oproj_publish_tail` 后，再用 `consume_head` CAS 领取 queue entry，才能读取对应的 `O_scratch`。
...
```text
O_proj task         = (row_key, n_super_group)
n_super_group       = 一组连续 out_n_tile
...
for k_chunk in K_local:
    A_chunk = view(O_scratch[row_tile_id, :, :, :])[:, k_chunk]
...
-----------------------------------------
FA tensor smem                      = 208 KB
```

3-stage FA 已经非常接近 227 KB 上限，留给 mbarrier/padding 的空间太小。第一版优先使用 2-stage FA，除非后续实测证明 3-stage 的收益足以抵消 occupancy 和实现风险。
...
按 `M_TILE=64, N_TILE=128, dtype=fp16/bf16`，如果：
...
```text
ar_tile = (row_key, n_super_group)
owner   = hash(row_tile_id, n_super_group) % tp_size
```
...
也就是说，ready count 的粒度是 `ar_slot_id = (row_tile_id, n_super_group)`，不是单个 `out_n_tile`。一个 rank 对同一个 `ar_slot_id` 只加一次 ready count；只有这个 super group 内所有 `valid_n_tiles` 都已经写入 symmetric partial buffer 后，才能参与 ready count。
...
解码 ar_tile = (row_key, n_super_group)
...
Y_final[row_tile_id, sg_tile, m, n]
...
每次 kernel/layer 的 control 状态:
    fa_task_counter
    fa_done_count
    head_ready_count[num_row_tiles]
...
local_owned_ar_tasks =
    count((row_tile_id, n_super_group)
          where owner(row_tile_id, n_super_group) == my_rank)
```
...
```text
FA 保持 (row_key, head) 粒度，保留 head 维并行度。
O_proj/AR 以 out_n_tile 为粒度，让 consumer WG 独占 tile。
O_proj CTA task 以 n_super_group 为粒度，super group 内每个 out_n_tile 仍由单个 consumer WG 独占。
...
```text
1. 先验证 workspace start init、local grid_sync、NVLink init barrier。
2. 再验证 FA task -> O_scratch -> head_ready_count -> O_proj ready queue。
3. 再验证 O_proj task 的单 rank GEMM 正确性。
4. 再接入 symmetric partial buffer 和 push-to-owner ready_count。
5. 再接入 last-arriver 投递的 AR owner ready bitset 和 ar_done_bits。
6. 再接入 NVLS multimem reduce/store。
7. 再验证 shared storage overlay 后的 FA mode / O_proj mode 切换收尾。
8. 最后验证 local_done -> NVLink exit barrier -> kernel return。
```
