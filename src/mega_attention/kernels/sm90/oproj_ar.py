...
class OProjARFusedKernelSM90:
    ...
    def __init__(
        ...
        self.threads_per_cta = (
            ...
            + self.num_comm_warp_groups
        ) * self.num_threads_per_warp_group
        self.load_warp_id = 0
        self.epi_store_warp_id = (
            self.num_dma_warp_groups * self.num_warps_per_warp_group
        )
        # First warp-group index of the comm group (DMA=0, MMA=1..,comm=last).
        self.comm_warp_group_idx = self.num_dma_warp_groups + self.num_mma_warp_groups
        self.load_register_requirement = 40
        self.mma_register_requirement = 232
        self.comm_register_requirement = 40
        ...
        self.comm_sync_barrier = pipeline.NamedBarrier(
            barrier_id=2, num_threads=self.num_comm_threads
        )
    ...
    def __call__(
        ...
        # MegaAttn: build multicast (NVLS) views of C and the flag buffer from
        # their raw symmetric-memory multicast virtual addresses. multimem.* PTX
        # operates on these multicast pointers; the NVSwitch fans the access out
        # to every rank's copy and (for ld_reduce) sums them in-network.
        c_mc_iter = cute.make_ptr(
            c.element_type, c_mc_ptr, cute.AddressSpace.gmem, assumed_align=16
        )
        c_mc = cute.make_tensor(c_mc_iter, c.layout)
        flag_mc_iter = cute.make_ptr(
            flag.element_type, flag_mc_ptr, cute.AddressSpace.gmem, assumed_align=4
        )
        flag_mc = cute.make_tensor(flag_mc_iter, flag.layout)
    ...
    def kernel(
        ...
        is_comm_warp_group = warp_group_idx >= self.comm_warp_group_idx
        is_mma_warp_group = (not is_dma_warp_group) and (not is_comm_warp_group)
        if is_dma_warp_group:
            cute.arch.setmaxregister_decrease(self.load_register_requirement)
        if is_comm_warp_group:
            cute.arch.setmaxregister_decrease(self.comm_register_requirement)
        ...
        if is_comm_warp_group:
            ...
            tiled_copy_cmc = cute.make_tiled_copy_tv(
            ...
            comm_work_tile = comm_tile_sched.initial_work_tile_info()
