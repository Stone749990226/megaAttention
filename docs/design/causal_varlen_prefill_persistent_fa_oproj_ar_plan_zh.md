...
本文的第一版范围和 invariant；如果 FA4 通用路径与本文冲突，以本文设计为准，先讨论再修改。
...
很少，主要并行度来自 head、KV split 或 paged cache；这些机制不进入第一版 fused
O_proj/AR 验证路径。
...
第一版支持完整 prompt prefill 和 Q rows 较多的 contiguous-KV chunked/append prefill。
...
`cu_seqlens_q/cu_seqlens_k` 计算。第一版要求 host 或 wrapper 在 launch 前保证每个
...
`valid_fa_m` 等可由 `cu_seqlens` 直接推出的信息。由于第一版支持 `q_len < k_len`，
...
第一版 causal block range 采用 FA4 `BlockInfo.get_n_block_min_max` 的同构公式。
...
FA 的 KV block loop 使用 runtime 计数，不使用 `range_constexpr(nblk)`。第一版仍推荐
从最右侧可见 K block 向左处理：
...
第一版验证完整 prompt prefill 与 Q rows 较多的 contiguous-KV chunked/append prefill，
但仍暂不处理：
