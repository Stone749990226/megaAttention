...
N_TILE = 128
total_q_tokens = 64k
num_fa_row_tiles = total_q_tokens / OPROJ_M_TILE = 512
super_group_n_tiles = 4
```
...
每个 rank 只分配自己 owner 的控制状态。按 `ceil(total_oproj_tasks / tp_size)`
...
`ar_ready_bits` 里的 bit 出现时，该 owner-local slot 已经 ready。当前设计 owner_words_alloc
...
当前设计不使用有界探测（`max_probe_words` + active/drain 区分）。引入它的唯一动机是 owner_words
很大时避免 CTA 长时间空扫，但当前设计 owner_words ≤ ~32，一次全扫描只是几十次 acquire load，
远小于一次 mode 切换的 overlay drain；而且全扫描消除了「ready bit 已置但本轮探测窗口扫不到」
造成的认领延迟——AR 是决定 kernel 退出的末级 task，认领延迟会直接拖长尾部。owner_words 规模
很大时才重新评估有界探测；当前路径不使用有界探测。
...
### Workspace 预分配与 kernel start 定向清理
...
launch_fused_fa_oproj_ar(..., workspace=workspace)
```
...
- 对 `C_sym`、`ready_count_owner`、`ar_ready_bits`、`nvl_barrier_state` 等
  symmetric allocation 做 `torch.distributed._symmetric_memory.empty` 和
  `rendezvous`。
- 保存 `C_sym` multicast pointer、NVLink barrier pointer、每个 rank 的
  `ready_count_owner` / `ar_ready_bits` peer pointer。
- workspace 创建时完成一次初始化清零和跨 rank rendezvous；后续每层 launch 不再 host reset。
...
kernel start 只定向清 active range，不清整块 capacity。后续更大的 batch 会在自己的
active init 中清到更大的范围。`owner_words_active` 的最后一个 word 整个清零，用来
覆盖 owner tail bit，避免上一层尾部残留伪 ready。
...
每次 kernel/layer 的 control 状态:
    fa_task_counter
    fa_done_count
    head_ready_count[max_num_row_tiles]        # 清 active prefix，按 FA tile（128 行）粒度
    ...
    ready_count_owner[max_owner_slots]         # 清 active prefix，owner-local ready count
    ar_ready_bits[max_owner_words]             # 清 active prefix，owner-local ready bitset
    ar_done_bits[max_owner_words]              # 清 active prefix，owner-local terminal protection
    ar_scan_cursor
    ar_done_count
```
...
task_ctrl:
    ...
    kernel start cleaner 只清 task_ctrl 和 per-layer control arrays。

sync_ctrl:
    长期 barrier state。
    包含 grid_sync_count[]、nvl_barrier_counter、nvl_barrier_signal[2]。
    不进入 kernel start cleaner 的清理范围。
```
...
每次 kernel/layer start 必须把本次 active 范围内的 per-layer control 状态清成 0：
...
arrays:
    head_ready_count[0 : num_row_tiles_active]
    ready_count_owner[0 : owner_slots_active]
    ar_ready_bits[0 : owner_words_active]
    ar_done_bits[0 : owner_words_active]
```
...
C_sym:
    ...
    owner AR 只 reduce/store 有效 m/n，或保证 padding 元素不会被后续逻辑读取。
...
kernel start 清理由所有 persistent CTA 分摊，而不是用一个 CTA 串行 memset：

```text
global_tid = cta_idx * block_threads + thread_idx
stride     = num_persistent_ctas * block_threads

for i in range(global_tid, num_row_tiles_active, stride):
    head_ready_count[i] = 0

for i in range(global_tid, owner_slots_active, stride):
    ready_count_owner[i] = 0

for i in range(global_tid, owner_words_active, stride):
    ar_ready_bits[i] = 0
    ar_done_bits[i] = 0
...
清理阶段没有并发 producer/consumer：上一层 `nvl_barrier(exit)` 已保证旧读写完成，本层
`nvl_barrier(init)` 前 persistent scheduler 还没开始。因此清理使用普通 global store
即可，不需要 atomic。清理 store 之后必须进入 local `grid_sync`，再进入跨 rank
`nvl_barrier(init)`，把本 rank 的清理完成发布给其它 rank。

每层 kernel 的生命周期：

```text
1. kernel start init:
    所有 CTA 并行清理本 rank active per-layer control state
    local grid_sync(init_clean_done)
    nvl_barrier(init_clean_done_across_ranks)
...
当前设计正确性依赖 kernel start 定向清理，不依赖尾部 cleanup overlap。后续可以做不改变
correctness boundary 的 in-flight 小优化：

```text
head_ready_count[row_tile]:
    最后一个 FA head 发布 O_proj tasks 后，该 row 的 head_ready_count 可以置 0。

ready_count_owner[owner_idx]:
    owner AR 完成 reduce/store 后，该 owner slot 可以置 0。

ar_ready_bits:
    owner claim 时通过 atomic_and 清 bit，本身已经是自清理。
```

这些优化只能减少下一次 start init 的清理工作量，不能替代 start init。当前设计不引入
独立 `MODE_CLEAN` 或尾部 cleanup scheduler，避免和 AR owner reduce 抢 persistent CTA
以及增加 termination 协议复杂度。
...
当前设计为 kernel-level init/exit barrier 固定使用两个 grid sync counter：
...
两个 counter 不复用。虽然 `grid_sync` 自身通过最高位翻转表达 phase，但 init 和 exit
是不同 barrier 点，固定分开能避免长期 workspace 复用时不同 barrier 点之间的状态混淆。
...
当前设计中，`nvl_barrier(init)` 和 `nvl_barrier(exit)` 都固定包含前后两个 local
`grid_sync`。不提供跳过 prologue/epilogue grid sync 的 kernel 内变体，避免某些 CTA
仍在访问 control/symmetric workspace 时 CTA0 提前发出跨 rank signal。
...
即使当前 kernel 只有 init 和 exit 两次跨 rank barrier，也采用 phase/sign 版本。这样 workspace 可以跨层复用，不依赖 host 每层清 barrier signal，也方便加入更多全局 barrier。
