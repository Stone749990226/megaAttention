...
- causal mask 和完整 prompt prefill 语义。
...
限制为 Hopper SM90、causal、varlen prefill，以及
FA + O_proj + NVLS AllReduce fused persistent kernel。

不得因为 FA4 支持或包含以下路径，就把它们引入本项目：
...
直接引入本项目 Hopper SM90 causal varlen prefill FA + O_proj + NVLS AR fused kernel。
...
src/mega_attention/kernels/sm90/fused_fa_oproj_ar.py
    最终 fused persistent kernel。已按设计文档把 real FA、real O_proj、
    real NVLS AR 接入同一个 persistent scheduler（tp_size=1 / 8×H200 均验证通过）。
...
开发与验证目标环境是多卡 Hopper SM90 + NVSwitch/NVLS。任何 kernel 正确性、hang 修复、
性能结论都必须基于实际 Hopper 环境验证。没有实际运行对应测试时，不要声称 kernel 已经通过。
...
当你进行代码上的修改后，需要运行 benchmark 测试与修改前进行对比验证。
...
是否仍满足 Hopper SM90、causal、varlen prefill 的范围？
是否需要回到核心设计文档确认？
```
...
当需要修改设计文档时，不要使用任何模棱两可的句子，比如“推荐使用”/“建议”等语句，而是该怎么设计就直接写清楚，如果有取舍和矛盾需要停下来与我确认。
