...
#
# The two cannot share a process: megaattn uses system /usr/bin/python + cutlass-dsl
# + torch symmetric memory; triton_dist uses a separate venv + a Triton fork + NVSHMEM.
...
/usr/bin/python -m torch.distributed.run --nproc_per_node=8 bench.py \
