...
# (oproj_ar.py :: OProjARFusedKernelSM90).
#
...
#
# Each pass re-runs benchmarks/bench_oproj_ar.py (python import + NCCL init + CuTe-DSL JIT compile),
...
#
# Usage:
#   ./ncu_oproj_ar.sh [-n N] [-o OUTDIR] [-p MASTER_PORT] [extra benchmarks/bench_oproj_ar.py args]
# Examples:
...
BENCH_SCRIPT="${PROJECT_DIR}/benchmarks/bench_oproj_ar.py"
...
# (cold L2 + symmetric-mem first-touch inflate long_scoreboard). benchmarks/bench_oproj_ar.py fires
...
while (( "$#" )); do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: $0 [-n N] [-o OUTDIR] [-p MASTER_PORT] [--section 1|2a|2b|2c] [extra bench_oproj_ar.py args]"
...
# Ensure benchmarks/bench_oproj_ar.py gets the matching --prof-warmup unless the caller set one.
if ! printf '%s\n' "${bench_args[@]}" | grep -q -- '--prof-warmup'; then
  bench_args+=(--prof-warmup "$PROF_WARMUP")
fi

# Distributed env that benchmarks/bench_oproj_ar.py / dist.init_process_group("nccl") need
...
pkill -9 -f "$BENCH_SCRIPT" 2>/dev/null
...
for (( i=0; i<NUM_PROCESSES; ++i )); do
  RANK=$i LOCAL_RANK=$i "$PYTHON" "$BENCH_SCRIPT" "${bench_args[@]}" \
...
# Each rank picks its own device via LOCAL_RANK.
for (( i=0; i<NUM_PROCESSES; ++i )); do
  RANK=$i LOCAL_RANK=$i \
...
while :; do
  ...
  if (( now - last_change > STALL_LIMIT )); then
    ...
    pkill -9 -x ncu 2>/dev/null; pkill -9 -f "$BENCH_SCRIPT" 2>/dev/null
    rc=124; break
  fi
  sleep 15
done
