...
kernel. Unlike benchmarks/bench_oproj_ar.py it does NOT run the torch-GEMM / NCCL-AR baseline loops —
...
deadlock (which kernel-replay isolation would cause).
  ...
  ncu --target-processes all --replay-mode application \
      ...
      python -m torch.distributed.run --nproc_per_node=8 scripts/prof_driver.py
"""
import os
import torch
...
from mega_attention.reference import oproj_ar as ref
from mega_attention.kernels.sm90.oproj_ar import OProjARFusedKernelSM90
...
def main():
    ...
    # benchmarks/bench_oproj_ar.py's working timing path.
    C.zero_()
    flag.zero_()
